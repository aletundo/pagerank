...
def init_argument_parser():
    ...
    parser.add_argument('--nodes', default=50, type=int,
                        help='Graph nodes number')
    parser.add_argument('--edge_probability', default=0.025, type=float,
...
def apply_power_method(H, iterations):
    ...
    LOGGER.info("I = {}".format(initial_eigenvalue))
...
def compare_algorithm(graph, dumping_factor, iterations, I, seed):
    ...
    LOGGER.info("Naive PageRank:")
    LOGGER.info(sorted_naive_pagerank)

    LOGGER.info("NetworkX PageRank:")
    LOGGER.info(sorted_networkx_pagerank)
...
def show_plots(G, I, I_networkx, seed):
    ...
    nx.draw(G, ax=ax1, cmap=plt.get_cmap('Reds'), node_color=I, with_labels=True, font_weight='bold',
            pos=nx.spring_layout(G, seed=seed))
    ax2.set_title("NetworkX")
    nx.draw(G, ax=ax2, cmap=plt.get_cmap('Reds'), node_color=I_networkx, with_labels=True, font_weight='bold',
            pos=nx.spring_layout(G, seed=seed))
    plt.show()
